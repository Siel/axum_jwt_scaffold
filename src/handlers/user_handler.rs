...
use crate::{
    auth::jwt_auth::JWTAuthMiddleware,
    auth::token::{self, TokenDetails},
    models::user,
    models::user::schema::{LoginUserSchema, RegisterUserSchema, User},
    views::user_view::FilteredUser,
    AppState,
};
...
pub async fn register_user_handler(
    ...
    if body.password != body.passwrod_confirm {
    ...
    let user_exists: Option<bool> = user::exists(&body.email, &data.db).await.map_err(|e| {
        let error_response = serde_json::json!({
            "status": "error",
    ...
    let user = user::new(&body.name, &body.email, &hashed_password, &data.db)
        .await
        .map_err(|e| {
            let error_response = serde_json::json!({
                "status": "error",
...
pub async fn login_user_handler(
    ...
    let user = user::get_by_email(&body.email, &data.db)
        .await
        .map_err(|e| {
            let error_response = serde_json::json!({
                "status": "error",
...
pub async fn refresh_access_token_handler(
    ...
    let user = user::get_by_id(&user_id_uuid, &data.db)
        .await
        .map_err(|e| {
            let error_response = serde_json::json!({
                "status": "fail",
